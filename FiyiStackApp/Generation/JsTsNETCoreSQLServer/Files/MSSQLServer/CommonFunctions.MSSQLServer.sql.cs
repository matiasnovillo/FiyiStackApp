...
namespace FiyiStackApp.Generation.JsTsNETCoreSQLServer.Files.MSSQLServer
{
    public static class CommonFunctionsMSSQLServersql
    {
        public static void Create(Project Project)
        {
            try
            {
                string Content = $@"USE [fiyistack_{Project.Name}]
GO
...
";
                ...
                string Path = $"{Project.Path}\\SQLScripts\\";
                if (!Directory.Exists(Path))
                { Directory.CreateDirectory(Path); }

                WinFormConfigurationComponent.CreateFile(
                $"{Path}CommonFunctions.MSSQLServer.sql",
                Content,
                true);
            }
